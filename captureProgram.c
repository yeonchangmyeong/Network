...
void *PacketCapture_thread(void *arg) {
    ...
    sprintf(filename, "allFile");		/*(%d-%d-%dT%d:%d:%d).txt, tm.tm_year - , tm.tm_mon + , tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);*/
...
void Ethrenet_header_fprint(FILE *captureData, struct ethhdr *etherHeader) {
	...
	char filename6[40];
    	sprintf(filename6, "captureFile");               /*(%d-%d-%dT%d:%d:%d).txt, tm.tm_year - , tm.tm_mon + , tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);*/
    	FILE *ethipData = fopen(filename6, "a+");  //파일 이어서 작성
       
    fprintf(ethipData, "0x%02X00\n",
            etherHeader->h_proto);  // L3 패킷 타입 IPv4 : 0x0800  | ARP 패킷 : 0x0806 | VLAN Tag : 0x8100
    fprintf(ethipData, "[%02x:%02x:%02x:%02x:%02x:%02x]\n",  // 6 byte for src
...
void Ip_header_fprint(FILE *captureData, struct iphdr *ipHeader, struct sockaddr_in source, struct sockaddr_in dest) {
   ...
   fprintf(captureData, "          |                       IP Header                        |\n");
    ...
    fprintf(captureData, "		IP Header Length	|	%dDWORDS(%dBytes)\n", (unsigned int)ipHeader->ihl,
            ((unsigned int)(ipHeader->ihl)) * 4);
    fprintf(captureData, "		Type Of Service		|	%d\n", (unsigned int)ipHeader->tos);
    fprintf(captureData, "		IP Total Length		|	%dBytes\n", ntohs(ipHeader->tot_len));
   fprintf(captureData, "           --------------------------------------------------------\n");
...
char filename4[40];
    sprintf(filename4, "TCP");               /*(%d-%d-%dT%d:%d:%d).txt, tm.tm_year - , tm.tm_mon + , tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);*/
    FILE *TcpData = fopen(filename4, "a+");  //파일 이어서 작성
  ...
  //  fprintf(captureData, "           --------------------------------------------------------\n");
   ...
   // fprintf(captureData, "           --------------------------------------------------------\n");
    fprintf(TcpData, "%dDWORDS(%dBYTES)\n", (unsigned int)tcpHeader->doff,
   ...
   // fprintf(captureData, "           --------------------------------------------------------\n");
    ...
    fprintf(captureData, "           --------------------------------------------------------\n");
    fprintf(captureData, "		OFFSET(Header Length)	|	%dDWORDS(%dBYTES)\n", (unsigned int)tcpHeader->doff,
            (unsigned int)tcpHeader->doff * 4);
    fprintf(captureData, "           -- FLAGS -----------------------------------------------\n");
    ...
    fprintf(captureData, "		Window Size		|	%d\n", ntohs(tcpHeader->window));
    fprintf(captureData, "           --------------------------------------------------------\n");
    fprintf(captureData, "		Checksum		|	0x%04x\n", ntohs(tcpHeader->check));
...
void Udp_header_fprint(FILE *captureData, unsigned char *Buffer, struct ethhdr *etherHeader, struct iphdr *ipHeader,
    ...
    char filename5[40];
    sprintf(filename5, "UDP");               /*(%d-%d-%dT%d:%d:%d).txt, tm.tm_year - , tm.tm_mon + , tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);*/
    FILE *UdpcaptureData = fopen(filename5, "a+");  //파일 이어서 작성
  ...
  //  fprintf(captureData, "          |                       UDP Header                       |\n");
   // fprintf(captureData, "           --------------------------------------------------------\n");
    fprintf(UdpcaptureData, "%u\n", ntohs(udpHeader->source));
    fprintf(UdpcaptureData, "%u\n", ntohs(udpHeader->dest));
   // fprintf(captureData, "           --------------------------------------------------------\n");
    fprintf(UdpcaptureData, "%d\n", ntohs(udpHeader->len));
    fprintf(UdpcaptureData, "0x%04x\n", ntohs(udpHeader->check));
   // fprintf(captureData, "           --------------------------------------------------------\n");
...
void Icmp_header_fprint(FILE *captureData, unsigned char *Buffer, struct ethhdr *etherHeader, struct iphdr *ipHeader,
    ...
    Ip_header_fprint(captureData, ipHeader, source, dest);  // ip 정보 print
	char filename3[40];
    sprintf(filename3, "ICMP");               /*(%d-%d-%dT%d:%d:%d).txt, tm.tm_year - , tm.tm_mon + , tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);*/
    FILE *ICMPDATA = fopen(filename3, "a+");  //파일 이어서 작성
    fprintf(ICMPDATA, "(IcmpPacket)%d\n", (unsigned int)(icmpHeader->type));
    fprintf(ICMPDATA, "%d\n", (unsigned int)(icmpHeader->code));
    fprintf(ICMPDATA, "0x%04x\n", ntohs(icmpHeader->checksum));
...
void Change_hex_to_ascii(FILE *captureData, unsigned char *data, int op, int Size) {
   char filename2[40];
    sprintf(filename2, "PayLoad");               /*(%d-%d-%dT%d:%d:%d).txt, tm.tm_year - , tm.tm_mon + , tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);*/
    FILE *TcpcaptureData = fopen(filename2, "a+");  //파일 이어서 작성
	//cmd에 ascill로 출력 
    if (op == A) {
    ...
    //file에 write하는 경우
        ...
        fprintf(TcpcaptureData, "\nDATA (Payload)\n");
        for (int i = 0; i < Size; i++) {
            if (i != 0 && i % 16 == 0) {          // 16개 데이터 출력 했다면, ascii코드 출력후 개행후 이어서 출력
                fprintf(TcpcaptureData, "\t\t");     // 16진수 data랑 ascii data 구분
                for (int j = i - 16; j < i; j++)  // 16진수 data를 ascii로 변환
                {
                    if (data[j] >= 32 && data[j] < 128)
                        fprintf(TcpcaptureData, "%c", (unsigned char)data[j]);  // data가 ascii라면 출력

                    else
                        fprintf(TcpcaptureData, ".");  //그외 데이터는 . 으로 표현
                }
                fprintf(TcpcaptureData, "\n");
            }

            if (i % 16 == 0) fprintf(TcpcaptureData, "\t");  //가시성을 위해 처음 오는 data는 tab

            fprintf(TcpcaptureData, " %02X", (unsigned int)data[i]);  //앞의 빈자리 0으로 초기화한 16진수로 데이터 출력

            if (i == Size - 1)  //마지막 data
            {
                for (int j = 0; j < (15 - (i % 16)); j++)
                    fprintf(TcpcaptureData, "   ");  //마지막 데이터는 16개 꽉 안채울 수 있으니 데이터 포맷을 위해 남은 공간만큼 space

                fprintf(TcpcaptureData, "\t\t");  // 16자리 까지 공백 채운후 ascii 출력 위해 구분

                for (int j = (i - (i % 16)); j <= i; j++)  //남은 데이터 ascii로 변환
                {
                    if (data[j] >= 32 && data[j] < 128)
                        fprintf(TcpcaptureData, "%c", (unsigned char)data[j]);
                    else
                        fprintf(TcpcaptureData, ".");
                }
                fprintf(TcpcaptureData, "\n");
